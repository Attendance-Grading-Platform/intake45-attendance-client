...
import { createAnnouncement } from '@/api/modules/announcement.api'
...
// Local Storage Key
const DRAFT_STORAGE_KEY = 'track_admin_announcement_draft'
...
onMounted(async () => {
  try {
    // 1. Fetch allowed cohorts for this track admin
...
const loadHistory = async () => {
  ...
  try {
    // Track admin uses the generic /me/announcements endpoint
    const res = await api.get('/v1/me/announcements')
        ...
        <div class="rounded-card border border-neutral-300 bg-neutral-0 p-6 flex flex-col h-full shadow-sm">
          ...
          <div class="mb-5">
            ...
            <select
              ...
              class="w-full rounded-btn border-neutral-300 bg-neutral-50 px-3 py-2 text-sm shadow-sm focus:border-brand-red focus:ring-brand-red transition-colors"
            >
          ...
          <div class="mb-5 border-b border-neutral-200 pb-2">
            <input
              ...
              class="w-full border-none bg-transparent font-serif text-3xl font-bold text-neutral-900 placeholder:text-neutral-300 focus:outline-none focus:ring-0"
            />
          </div>

          <!-- TipTap Toolbar -->
          <div class="flex items-center gap-2 mb-2 bg-neutral-50 p-2 rounded-t-md border border-neutral-200 border-b-0" v-if="editor">
            <button
          ...
          <!-- Editor Content -->
          <div class="flex-1 border rounded-b-md border-neutral-200 bg-white">
        ...
        <div class="rounded-card border border-neutral-300 bg-neutral-50 p-6 sticky top-24">
          ...
          <ul class="space-y-4 text-sm text-neutral-600 font-sans">
            <li class="flex items-start gap-2">
              ...
              <span><strong>Track Admin Privileges:</strong> You can post announcements to any of your cohorts at any time.</span>
            </li>
            <li class="flex items-start gap-2">
              <span class="text-brand-red mt-0.5">•</span>
          ...
          <tbody class="divide-y divide-neutral-200">
            ...
            <tr v-for="item in history" :key="item.id" class="hover:bg-neutral-50 transition-colors">
              ...
              <td class="py-4 px-6 font-sans text-sm text-neutral-600">
                <!-- Using author as a fallback if cohort name not available immediately, but typically we want the cohort name if we fetched it, or just display the id if it's all we have from myAnnouncements -->
                Cohort ID: {{ item.cohort_id }}
              </td>
