...
- [ ] `users` — id, name, email, password, role (enum), compensation_type (nullable), hourly_rate (nullable), salary (nullable), expires_at, track_id (nullable FK)
- [ ] `tracks` — id, name, branch_id
- [ ] `cohorts` — id, track_id (FK), name (e.g. "Web — Intake 45"), status (active/closed), started_at, ended_at
- [ ] `courses` — id, cohort_id (FK), name, total_weight (always 100)
- [ ] `course_components` — id, course_id (FK), name (lab_deliverables / final_exam_project), weight (must sum to 100 per course)
...
- [ ] **LC-1** A track has at most ONE active cohort at any time — enforce this
- [ ] **LC-2** Only Branch Manager can create cohorts and assign Track Admins
- [ ] Implement cohort stages: Open → Configure → Deliver → Participate → Roll Up
- [ ] Branch Manager creates cohort and assigns Track Admin(s)
...
- [ ] `GET  /api/tracks`
- [ ] `POST /api/cohorts` — Branch Manager only
- [ ] `GET  /api/cohorts/{id}`
- [ ] `PUT  /api/cohorts/{id}`
